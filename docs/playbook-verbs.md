...
Alongside Run, a Playbook selection offers **verbs**: typed refinement actions
...
Select text in the Playbook to open the selection context menu. It offers
...
A verb spawns a dedicated session forked from the Playbook's owning session —
...
session can read the Playbook but has **no tool that can edit it**; it
...
- Type to answer — keystrokes go to the verb session, not the Playbook
...
- **Unpin** by clicking the same clip again, or by clicking anywhere else —
  in the Playbook body, its chrome, or outside the modal.
...
the Playbook. When the session *is* visible elsewhere (a split pane, say),
...
|---|---|---|
| `name` | No | Stable id, referenced by `construct_playbook_verb_execute`. Defaults to the file's own name (`threat-model.md` → `threat-model`) — most verbs need no `name:` line at all. |
...
| Placeholder | Value |
|---|---|
| `{{ playbook.content }}` | The full Playbook document (bounded in size; truncated copies point back at the live read tool). |
| `{{ playbook.selected_text }}` | The selection itself. |
| `{{ playbook.additional_instruction }}` | The free-text instruction typed alongside the verb, empty if none. |
...
`0089-playbook-selection-verbs.md` (verb lifecycle, merge semantics, the
markdown-file registry) and `0090-playbook-clip-pin-interactive-terminal.md`
