...
| Capability | smith | claude | codex | opencode | kimi | hermes | pi | prime-agent | muse | grok | antigravity | shell |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Interactive / headless | both | both | both | interactive | interactive | both | both | both | both | both | both | PTY |
...
| MCP injection | native tools | ✓ | ✓ | ✓ | gap | gap | gap⁷ | gap⁷ | gap | gap | gap | — |
...
| prime-agent | `<CONSTRUCT_SESSION_DATA_DIR>/prime-agent-sessions/<file-uuid>.jsonl` (private store selected via `--session-dir`; the filename UUID differs from the native header UUID in 0.7.0) | Pi-compatible v3 message/event records, model and thinking changes, full token/cost usage, tool calls/results, and error diagnostics |
