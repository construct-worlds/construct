# 0173-service-definitions-apply-without-restart
...
A service definition is applied to the running daemon when it is saved. No
...
Definitions describe a running service, so an edit that requires a restart is
...
- A service that has been paused in configuration stops serving as soon as the
  daemon notices. Installations that paused a service while relying on it
  continuing to answer will see it stop.
- Adding a field to a service definition means assigning it a propagation
  class; a field with no class is an unanswered question at the point of edit.
- Per-service runtime state — which session serves which key, and which
...
- A service is paused. Its port is released and callers are refused. Resuming
...
- Two channels are given the same port by hand. The one already serving keeps
  ...
  applies.
- A definition is saved with a syntax error. Every service keeps running
