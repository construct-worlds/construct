...
- **It opens with working defaults.** A harness is preselected (a real, available one when possible), the working directory defaults to the current one, the briefing starts empty. The user can confirm immediately without filling every slot.
