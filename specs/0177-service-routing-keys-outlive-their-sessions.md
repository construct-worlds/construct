# 0177-service-routing-keys-outlive-their-sessions

Status: accepted
Date: 2026-08-01
Area: persistence
Scope: A service routing key that maps to a deleted session must open a new session rather than fail.

## Decision

A service's routing table is a cache of which session currently serves which
...
created, adopted by the service, and recorded under that key. The delivery
...
Sessions are deletable from every client, and a service is not consulted or
...
repair is hand-editing the service's state file. The channel is left silently
broken for one caller while the service still looks healthy.
...
This does not make services responsible for preserving a deleted conversation's
history, and it does not add deletion notifications from sessions to services.
