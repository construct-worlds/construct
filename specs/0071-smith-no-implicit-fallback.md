...
OAuth-subscription providers (`claude-oauth:`, `codex-oauth:`, `grok-oauth:`, `kimi-oauth:`) and Ollama remain fully supported — they just require an explicit `<prefix>:<model>` spec, via `--model`, `CONSTRUCT_SMITH_MODEL`, an `@profile`, or picking that method in the `/configure` dialog's smith-auth tab (which pins `CONSTRUCT_SMITH_MODEL` for you — see [[0070-smith-model-pin-persistence]]). None of those paths are guessed automatically when no model is specified at all.
...
**Minibuffer exception.** An ordinary user session still fails hard at start exactly as above. The minibuffer's smith session (`CONSTRUCT_SESSION_KIND=minibuffer`) does not: it starts its REPL regardless of whether a model resolved, because slash commands — including the fleet-dispatch ones the minibuffer exists for, e.g. `/construct restart` — never touch the provider at all. The curated error still fires immediately as a visible message in the minibuffer's panel; it just doesn't take the whole panel down with it. The first real turn that actually needs a model (plain text, not a slash command) re-attempts resolution at that point — so exporting a key or picking a method in `/configure` recovers the *same* minibuffer session without needing a restart, unlike the smith-auth pin in [[0070-smith-model-pin-persistence]] which does need one.
...
- Any future rung added to the ladder must be an explicit, deliberate choice about what "auto-detect" should guess — silently falling through to a network-dependent default is exactly the failure mode this decision closes off. A rung qualifies only if its credential is a direct API key that can be checked with a cheap, side-effect-free presence test, and it goes at the end of the ladder so existing machines keep resolving to the same provider.
...
- No API keys, no `CONSTRUCT_SMITH_MODEL`, no Ollama running: `construct new --prompt "..." smith` immediately errors with a message naming the missing credentials and pointing at `/configure`, instead of the session appearing to start and then failing on the first turn.
...
- A completely keyless machine boots the TUI: the minibuffer (minibuffer) session still comes up and shows the curated "no model configured" message, but `/construct restart`, `/loop`'s ToolCall-routed variants, and other slash commands keep working from it. The user exports `ANTHROPIC_API_KEY` in the daemon's shell and restarts the daemon; the *next* daemon-respawned minibuffer resolves normally. If instead they type a plain (non-slash) message to the still-running minibuffer without restarting anything, smith re-attempts resolution right then — if the daemon's own process env now has the key (e.g. it was already set but the daemon predates it), that same session recovers without a restart.
