# 0163-operator-endpoints-start-loopback-only

Status: accepted
Date: 2026-07-30
Area: architecture
Scope: The initial operator-endpoint ingress boundary.

## Decision

Operators are daemon-owned definitions. In v1 each operator is declared in its
own `operators/<name>.toml` file under the Construct configuration directory and
...
the channel-publication boundary in 0174. A operator owns its instruction,
harness, model, working directory, and routing policy; a channel supplies the
transport-specific credential and listener configuration. A operator may own
zero or more named channel instances. Channel ids are stable keys inside the
operator definition, and each HTTP channel owns its own loopback port,
credential, and enabled state. The HTTP port is therefore channel
configuration, not operator configuration; two HTTP channels on one operator
must use different ports. Channel definitions live in the daemon-wide
`channels.toml` catalog beside the `operators/` directory. A operator TOML's
`channels` map is its attachment state: attaching copies a catalog definition
into the operator, while detaching removes only that attachment and preserves
the catalog definition for later reuse. Channel ids are globally exclusive;
the catalog reports the owning operator so clients can disable an already-used
...
The TUI renders operators as top-level rows in the ordinary session list rather
than in a separate sidebar section. A operator row uses the distinct `◈` type
glyph, participates in the same scrolling, keyboard navigation, and pointer
selection as a session, and opens a first-class operator view in the active
split rather than a modal. Expanding or collapsing a operator's routed-session
children is a per-user TUI preference that survives client restarts; stale
preferences are discarded when their operator no longer exists. The view presents
...
the navigation bridge between operator configuration and the live conversation.

The operator view uses the same top-right title-actions affordance as a session
view. Its menu contains only pane actions—split or close the pane—and delete
the operator. Operator definition fields are edited directly in the focused
operator view; channel-specific editing and credential rotation belong to the
selected channel row, while pausing or resuming ingress is a operator field.
...
glyph, and a channel owned by another operator is dimmed with its owner shown.
...
at its top-left; clicking it or pressing Esc returns to the operator overview.
The routed-session list is a separate `Sessions` section
below the channel catalog. Global `C-x` chords remain available while the operator view is
...
the operator view's default key handling; operator-view commands resume when it
closes.
...
The WebTUI mirrors this surface at `/operators/<name>`. Operators appear in the
same list model as sessions, using the same `◈` glyph and row selection. Its
wide operator view uses the same definition/help columns, and narrow layouts
stack those columns. A operator may occupy a shared split pane alongside a
session; a pane identity therefore carries either a session id or a operator
...
channels do not bind listeners after restart; pausing a operator disables all
of its channels. Operator ingress acknowledges accepted work
...
operator-scoped result route. The result reports session status and the latest
...
and owner-control boundaries. Reusing ordinary sessions keeps operator work
visible and recoverable in the fleet.

## Consequences

- Operator keys and session mappings survive daemon restart.
- Operator ownership of keyed and per-event sessions survives daemon restart,
  so result reads remain scoped after recovery.
- Operator definitions can be created, edited, and removed atomically without
  rewriting the global Construct configuration.
- Channel mutations preserve other channels and the operator behavior fields;
  detaching the final channel leaves a valid but inert operator definition and
  keeps the channel available in the catalog.
- Channel catalog ownership is exclusive across operators. Detach is reversible
  and never destroys channel credentials; permanent catalog deletion is outside
  the v1 operator-view flow.
- Channel summaries never return bearer credentials. Creation and explicit
  rotation return a generated credential once, and ordinary operator edits do
  not replace it.
...
- Bearer credentials are operator-scoped and never logged.
- A tunnel route extends this boundary only through an explicit per-channel
  publication action; none is implied by enabling or attaching a operator.
- Operator views participate in shared split layout state across TUI and
  WebTUI, while keyboard/pointer focus remains client-local.
- TUI operator-row disclosure state is client-local and persists across
...
- V1 does not promise channel plugins or response streaming.
  ...
  operator definitions without a restart, which supersedes this consequence.

## Non-Goals

- Replacing the remote-control listener or its owner authentication.
- Publishing a operator automatically.
...
operator. Every delivery enters the same headless session, which remains visible
to the owner in the normal fleet UI. The caller polls
`GET /svc/<operator>/sessions/<session-id>` with the same bearer credential until
