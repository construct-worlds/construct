# 0175-operator-view-focus-is-edit-mode

Status: accepted
Date: 2026-08-01
Area: ux
Scope: Focusing a operator view puts it in edit mode, and unsaved changes are marked in the pane title.

## Decision

A operator view has exactly one interactive state: editing. Every path that
focuses a operator — creating one, opening one by name, clicking its pane, or
moving pane focus onto it — leaves the editor open on that operator. There is no
read-only operator view the user must step out of before typing, and no
"press a key to start editing" step. Moving focus to something that is not a
operator closes the editor.

Sidebar selection and pane focus remain distinct. Keyboard navigation that
moves the session-list selection onto a operator updates the active pane and
...
action moves focus to the already-prepared operator editor.

A command that hands the user a focused operator view must also dismiss the
...
A operator being created has never been saved, so it is unsaved from the moment
it appears; Escape discards the draft outright.

The pane title marks unsaved state with a trailing `*` on the operator name.
...
The operator pane is a form. Splitting it into a "viewing" state and an
...
command could leave a operator selected while the keystrokes still went to the
...
- Any new way of focusing a operator must open its editor. Keyboard traversal
...
- This says nothing about how operator definitions are stored, validated, or
...
- Creating a operator from the fleet panel leaves the new view focused with the
  cursor in its first field; typing edits it immediately, and the title shows
  the name with `*`.
- Editing an existing operator's instruction shows `*`; Escape restores the
  saved instruction and clears `*`; Escape again moves the cursor to the
  session list, with the operator view still open beside it.
