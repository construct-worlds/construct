...
- Merging a fork always takes its result into the parent and archives the
  fork. There is no separate dedicated merge chord: the action lives on the
  session-end prompt (`C-x k` / `dd` shows `[m] merge and archive` when the
  selected session is a fork) and on the session title menu. Lineage
  surfaces keep a direct-key shortcut for the same path (including discard
  there). Auto-archive-after-merge applies to any fork, regardless of which
