...
- `unsafe_auto` (presented to users as `always-approve`): Safe and Risky tools run without asking the user.

Approval prompts expose `a` for `auto_review` and `f` for `always-approve`. The wire value remains `unsafe_auto` for compatibility, but clients should present this mode as `always-approve`.
