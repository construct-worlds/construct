...
2. **Interact (optional).** Verbs declare an interaction policy. A `single-shot` verb runs to completion unattended. An `interactive` verb may hold a dialogue with the user inside its own session; the user reaches it through the session clip annotated into the Playbook (see below). Awaiting-input status flows to the minibuffer through the existing fleet-observation channel.
