...
The playbook refers to sessions by id. For an orchestrator's subagent, the playbook clip is the only affordance that can point the main view at it — there is no list row to click. Tying selection-validity to list membership (or to a "user session" kind) made such a click unstable: the click set the selection, but the next session-list refresh re-validated against the visible list, found no row, and reverted — which also popped the previously stashed playbook back open, so the switch appeared to "bounce back" to the playbook.
...
A fleet/orchestrator playbook shows clips for the orchestrator's subagents. Clicking such a clip reveals that subagent in the main view and keeps it there across session-list refreshes; the list shows no highlighted row because the subagent has none. The selection only changes again when the user navigates elsewhere or the subagent ends.
