...
so: that the turn is waiting on the operator, which tool it is waiting on, a
...
per service and defaults to waiting indefinitely, so an operator who has not
...
refusing work an operator would have approved, at the cost of requiring them
...
- The reported summary is written for the operator, not the caller; it may
...
- Timeouts are evaluated when a result is requested, so a caller that stops
  polling leaves the turn parked. The operator's own surfaces remain the way
...
- Notifying the operator through the channel the request arrived on.
...
- A request triggers a file write. The caller polls and sees that the turn is
  waiting on the operator, which tool it is, and that it has been waiting nine
  seconds. The operator approves; the next poll returns the finished reply.
