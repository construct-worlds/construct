...
- **Navigating the list keeps the caret on the list.** Selecting a session from the list's keymap must not pull focus into the session view, even though the incoming view would otherwise claim it. The TUI's list keeps its cursor across a switch, and that is precisely what makes next/previous *repeatable* — a selection that moves focus away ends the navigation after one step and forces the user back to the mouse. Only an explicit drill-in gesture moves focus to the view. This must hold across the whole switch, which is asynchronous: a surface that grabs focus late still breaks the next keystroke.
