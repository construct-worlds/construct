...
A reserved gesture the user cannot discover is not a solution, so the client must surface it at the moment it is needed. When a left-drag completes having been forwarded to the child — the one signal that distinguishes an attempted selection from ordinary clicking — the client tells the user that this pane's harness handles the mouse and that Shift-drag selects. The signal is necessarily ambiguous, because a child that uses drag itself (visual-mode selection, a draggable scrollbar) produces identical events from a user who wanted the drag forwarded; the hint is therefore shown once and then stays quiet, teaching the gesture without nagging.
...
- Any gesture the client reserves for itself over a mouse-grabbing pane must come with a way for the user to find it, because the failure mode is silent: the drag reaches the child, no selection appears, and nothing explains why the gesture that works everywhere else in the TUI did nothing here.
