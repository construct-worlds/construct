# 0048-playbook-shimmer-agent-contract

Status: accepted
Date: 2026-06-28
Area: harness
Scope: What an agent executing a playbook Run must do with per-block shimmer so the animation truthfully tracks unfinished work.

## Decision

Shimmer on a playbook block has exactly one meaning to the agent: **the block's work is still pending in this run — queued, in progress, or not yet done; outcome unknown.** No shimmer means the block is **settled — done, skipped, or no work needed.**
...
- **Planning pass first.** The agent's first playbook action on a Run — before doing or delegating any task — is one status-only edit whose `pending` map declares every still-pending block and whose `settle_others` flag clears every omitted block. Because shimmer is a declared per-block state (see `0053-playbook-shimmer-block-addressing`), this needs no textual edit. This is the only way no-work and skipped blocks stop shimmering promptly instead of riding the optimistic full-playbook shimmer until the turn ends.
...
- A playbook has three blocks. The agent's first action sends `pending: {A: \"Implementing\", B: \"Preparing delegation\"}` with `settle_others: true`. Block C is omitted and clears immediately without a text edit. A and B keep shimmering; C is calm within seconds of Run.
