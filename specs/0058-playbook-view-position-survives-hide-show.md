# 0058-playbook-view-position-survives-hide-show

Status: accepted
Date: 2026-06-28
Area: tui
Scope: The Playbook view restores its caret and scroll position when hidden and shown again.

## Decision

Hiding the Playbook view and showing it again for the same session must land on
...
dismissing the modal) followed by reopening the same session's playbook.

## Reason

The Playbook view's caret and scroll are ephemeral UI state held only by the
active popup. When the playbook is hidden the popup is dropped, and reopening
...
playbook, hides it to glance at the terminal, and reopens it expects to resume
where they were, not to be thrown back to the top.

## Consequences

- A hidden playbook's caret + scroll must be captured before the popup is
...
- This remembered position is kept separate from the map that drives split-window
  rendering of non-active playbooks. A fully-hidden playbook must not reappear in a
  split pane just because its position is remembered; conversely, remembering the
  position must not depend on the playbook still rendering anywhere.
...
- Switching the selected session between playbooks is a separate mechanism that
  already preserves full popup state (including unsaved edits); this decision is
  specifically about the hide→show cycle of a single session's playbook.

## Non-Goals

- Persisting caret/scroll across a daemon restart or client relaunch. Only the
  set of open playbooks is persisted; their in-view position is in-memory only.
