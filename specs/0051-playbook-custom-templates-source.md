...
- **Legacy migration is default-location only.** The one-time `canvas/templates` → `playbook/templates` rename runs only when no directory override is set. When an operator points the daemon at an explicit directory, the daemon treats it as operator-owned and never moves files into it.
