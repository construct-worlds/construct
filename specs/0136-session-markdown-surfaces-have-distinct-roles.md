...
Date: 2026-07-07
Area: architecture
Scope: Which Markdown surface — playbook, widget, or memory — a feature or piece of content belongs to.
...
- **Playbook — what to do.** The single, durable, runnable orchestration document, co-edited by the human and the owning agent. It carries intent and the orchestration state that future runs must read to continue the work correctly.
...
1. Would a future playbook run misbehave or lose context if this content were absent? It belongs in the playbook.
...
- Widget content is never interpreted as instructions. Only the playbook document (or a selected fragment of it) is runnable.
- The playbook never becomes an agent-only status feed. High-frequency status churn that only the human consumes routes to widgets, not through playbook versioning and merge machinery.
- Memory never carries transient task state, and neither the playbook nor widgets substitute for it: content that must inform future sessions is written to memory even when it also appears on another surface.
...
- Playbook state is limited to what runs consume. When the agent records progress in the playbook, that progress is orchestration state a future run reads; purely human-facing progress display belongs in a widget, possibly projecting the same playbook region.
- A proposal that moves content across a boundary (making widgets runnable, making the playbook agent-only, making memory session-scoped) requires superseding this spec, not a local exception.
...
- A plan of numbered steps the session should execute, with per-step results the next run builds on: playbook.
- A compact checklist mirroring that plan so the human can glance at progress, with a "pause" action link: widget — ideally a projection of the playbook region rather than a second copy.
- "This repository's integration tests flake under concurrent build load": memory.
- An agent wants to surface a blocking question during a run: the blocker is recorded on the playbook (a future run must see it) and may also be surfaced as a widget with action links for the human to answer quickly.
