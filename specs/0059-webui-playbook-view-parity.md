# 0059-webui-playbook-view-parity

Status: accepted
Date: 2026-06-28
Area: webui
Scope: The web UI exposes the per-session Playbook surface with functional parity to the TUI, built on the same daemon contract.

## Decision

Every session's Playbook is reachable and fully usable from the web UI, not only the TUI. The web Playbook surface is a third per-session view mode, peer to Terminal and Chat, available for any session (every session owns a Playbook). It must support, at functional parity with the TUI:
...
- **Run** the whole document, or the current text selection, dispatching the playbook to the owning session.
...
- **Templates**: an empty Playbook offers the available non-blank templates as one-click starting points.
...
Playbook is a primary collaboration surface (a shared space the user and agents edit and run together), but it had been authored only through the TUI. Remote-control and mobile users reach agentd through the web UI and could see neither the document nor its run state. Parity removes that gap. Reusing the existing daemon RPC and broadcast surface — which already serves web clients — means no new server contract is required; the gap was purely a missing client renderer.
...
- The web Playbook surface rides the existing playbook get/update/edit/execute/list-templates calls and the playbook-state broadcast. Changes to those contracts must keep the web client working, not just the TUI.
...
- A rich rendered-Markdown editor: the Playbook surface is a plain-text Markdown editor with overlays (clips, shimmer, find), matching the TUI's plain-text model, not a WYSIWYG view.
