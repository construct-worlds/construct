...
- The previous per-edit `shimmer: true` flag is replaced by the call-level per-block declaration list. Tool and protocol surfaces expose shimmer as block-id-keyed declarations, not as a boolean attached to an individual find/replace.
