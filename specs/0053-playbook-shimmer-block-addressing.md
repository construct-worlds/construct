...
- **Update** accepts an MCP `pending` map keyed by zero-based block index in the new Markdown; omitted blocks settle. The adapter expands it into the daemon's complete ordered declaration and maps that declaration to new stable refs.
...
**Human-authored edits are the exception, and carry pending state automatically.** A block id survives a semantic edit — only the epoch advances — so an edit whose actor is a human re-attaches the pending state, and the agent's tooltip, to the produced block by id. Fail-closed addressing exists to stop a *stale declaration* from landing on changed meaning; a person typing in a task while the agent works on it is not a stale declaration, and settling that block would be a lie about the work (see `0048-playbook-shimmer-agent-contract`). Nothing would re-light it either: the agent has already declared that block and has no reason to declare it again. An explicit declaration in the same call still wins, and agents remain fully explicit — this rule never applies to agent-authored writes.
...
- No write changes the shimmer of a stable-ref-addressed block it did not name, except: `keep_pending`, which names blocks by the edit output instead of by a ref the caller cannot know yet; and a human-authored edit, which carries a pending block's state onto the ref it produces, keyed by the block id that survived the edit.
