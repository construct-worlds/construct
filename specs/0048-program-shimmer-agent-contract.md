...
- **Planning pass first.** The agent's first program action on a Run — before doing or delegating any task — is one status-only edit whose `pending` map declares every still-pending block and whose `settle_others` flag clears every omitted block. Because shimmer is a declared per-block state (see `0053-program-shimmer-block-addressing`), this needs no textual edit. This is the only way no-work and skipped blocks stop shimmering promptly instead of riding the optimistic full-program shimmer until the turn ends.
...
- A program has three blocks. The agent's first action sends `pending: {A: \"Implementing\", B: \"Preparing delegation\"}` with `settle_others: true`. Block C is omitted and clears immediately without a text edit. A and B keep shimmering; C is calm within seconds of Run.
