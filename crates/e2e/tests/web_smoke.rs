...
async fn web_client_loads_and_websocket_connects() {
        ...
        .evaluate(
            ...
            (async () => {
              ...
              const handle = {
                ...
                xtermUserInputSignalAvailable: true,
              };
              ...
              try {
                ...
                installTerminalInputOwnershipTracking(host, handle, null);
    ...
    assert_eq!(
        ...
        "web hover and protocol replies must be passive while drag input claims"
    );

    // Exercise the bundled xterm's real source signal: typed input is marked
    // as user-originated, while a DSR response generated by parsing child
    // output is not. Claude emits terminal queries during its interactive UI,
    // so treating those replies as input would let an idle browser reclaim
    // geometry without a click or key.
    let xterm_input_sources: serde_json::Value = page
        .evaluate(
            r#"
            (async () => {
              const saved = {
              ...
              let geometryClaims = 0;
              const host = document.createElement("div");
              host.style.cssText = "position:fixed;left:0;top:0;width:640px;height:320px";
              ...
              try {
                state.currentId = "s-real-xterm-sources";
                state.mode = "terminal";
                rpc = (method, params) => {
                  calls.push({ method, params });
                  return Promise.resolve(null);
                ...
                term.onData((value) => {
                  forwardTerminalData(handle, "s-real-xterm-sources", value);
                });

                term.input("typed", true);
                await new Promise((resolve) => term.write("\x1b[5n", resolve));
                ...
                await Promise.resolve();
                return {
                  signalAvailable: handle.xtermUserInputSignalAvailable,
                  geometryClaims,
                  inputClaims: calls.map((call) => call.params.claim),
                };
              } finally {
        ...
        .await
        .expect("evaluate real xterm input sources")
        .into_value::<serde_json::Value>()
        .expect("real xterm source result");
    assert_eq!(
        xterm_input_sources["signalAvailable"], true,
        "the bundled xterm must expose its user-input signal"
    );
    assert_eq!(
        xterm_input_sources["geometryClaims"], 1,
        "only real user input should claim geometry: {xterm_input_sources:?}"
    );
    assert_eq!(
        xterm_input_sources["inputClaims"],
