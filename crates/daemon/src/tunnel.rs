        ...
        while let Ok(Some(line)) = lines.next_line().await {
            ...
            if !announced {
                if let Some(https_url) = extract_trycloudflare_url(&line) {
                    ...
                    scan_remote.set_tunnel_url(Some(ws_url)).await;
                    announced = true;
                }
            }
...
fn print_qr(content: &str) {
    let code = match qrcode::QrCode::new(content) {
        Ok(c) => c,
        Err(e) => {
            tracing::warn!(error = %e, "failed to render QR code; skipping");
            return;
        }
    };
    // `Dense1x2` packs two QR modules per terminal cell vertically
    // using half-block characters, so the rendered code is roughly
    // square in a typical 1:2-aspect-ratio terminal cell.
    let rendered = code
