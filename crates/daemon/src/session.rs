...
fn canvas_run_instructions() -> Vec<String> {
    vec![
        ...
        "Smart clips are Markdown-native typed references. The clip_id attribute identifies a specific clip instance, not the target itself; preserve clip_id values when editing existing clips. Only insert a new clip into the canvas when the canvas content or the user's instructions explicitly call for one — do not add clips speculatively just because a subagent was created.".to_string(),
