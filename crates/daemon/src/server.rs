...
use construct_protocol::{
    ...
    TokenHistoryParams, UsageQueryParams, IPC_VERSION,
};
...
fn forward_broadcast(
    ...
    let notif = match msg {
        ...
        BroadcastMsg::GroupState(g) => {
            ...
            Notification::new(ipc_notif::PROJECT_STATE, Some(p))
        }
        BroadcastMsg::GroupDeleted(g) => {
            ...
            Notification::new(ipc_notif::PROJECT_DELETED, Some(p))
        }
...
async fn dispatch(
    ...
    dispatch_entry!(ipc_method::TOKEN_HISTORY, {
    ...
    dispatch_entry!(ipc_method::SESSION_SET_ROUTE, {
        ...
        match manager.set_route(&p.session_id, p.route, p.model, p.effort).await {
            Ok(()) => Response::ok(req.id.clone(), serde_json::Value::Null),
            Err(e) => Response::err(req.id.clone(), ErrorObject::internal(e.to_string())),
        }
    });
