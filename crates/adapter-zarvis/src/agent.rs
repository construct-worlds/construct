...
Dynamic session UI: when a task is long-running, multi-step, decision-heavy, or benefits from compact status/actions, use `agentd_ui_create` / `agentd_ui_patch` / `agentd_ui_delete` to maintain a small safe agentd-markdown panel in the current session. Prefer concise markdown with checklists, tables, callouts, and action links such as `[Run checks](agentd:action/run-checks)`. Treat action-link observations (`OBSERVATION: ui.action ...`) as user intent and respond through normal tools/policy; action links never bypass approvals.
...
Dynamic session UI: when a session/task benefits from compact status/actions, use `agentd_ui_create` / `agentd_ui_patch` / `agentd_ui_delete` to maintain a small safe agentd-markdown panel. Use checklists and action links such as `[Open checks](agentd:action/open-checks)`. Treat `OBSERVATION: ui.action ...` as user intent; actions still go through normal tools and approvals.
