...
/// Session-local Codex model catalog generated by the daemon router.
///
/// The Codex adapter consumes this internal handoff by adding a
/// `model_catalog_json` command-line config override. It is deliberately
/// not a public harness setting: users opt in through `[router]
...
fn mcp_env_toml_from(session_id: &str, lookup: impl Fn(&str) -> Option<String>) -> String {
    ...
    for name in CONSTRUCT_MCP_ENV_VARS {
        if let Some(value) = lookup(name) {
            pairs.push(format!("{name} = {}", toml_quote(&value)));
        }
    }
...
mod tests {
    ...
    fn mcp_env_toml_includes_context_env() {
        let got = mcp_env_toml_from("s123", |name| match name {
            ...
            agent_context::ENV_PLAYBOOK_RUN_CONTEXT_FILE => {
                Some("/tmp/playbook-run-context.json".to_string())
            }
        ...
        assert!(
            got.contains("CONSTRUCT_PLAYBOOK_RUN_CONTEXT_FILE = \"/tmp/playbook-run-context.json\"")
        );
